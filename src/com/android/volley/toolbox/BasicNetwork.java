...
public class BasicNetwork implements Network {
			...
			try {
				...
				if (statusCode == HttpStatus.SC_NOT_MODIFIED) {
					return new NetworkResponse(HttpStatus.SC_NOT_MODIFIED,
							new FileInputStream(
									request.getCacheEntry().dataFilePath),
				...
				return new NetworkResponse(statusCode, httpResponse.getEntity()
						.getContent(), responseHeaders, false);
				...
				try {
					...
					throw new NetworkError(illegalStateException);
	...
	private void logSlowRequests(long requestLifetime, Request<?> request,
		...
		if (VolleyLog.sDebug || requestLifetime > SLOW_REQUEST_THRESHOLD_MS) {
			VolleyLog.d(
					"HTTP response for request=<%s> [lifetime=%d], "
							+ "[retryCount=%d]", request, requestLifetime,
